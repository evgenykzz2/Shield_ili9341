...
void VideoDecoder::Decode4x4Primitive4C(const uint8_t* ptr)
{
	uint8_t l0 = ptr[0];
    uint8_t h0 = ptr[1];

	uint8_t l1 = l0 ^ (ptr[2] & 0xE7);
    uint8_t h1 = h0 ^ (ptr[2] & 0x18);
	
	uint8_t l2 = l0 ^ (ptr[3] & 0xE7);
    uint8_t h2 = h0 ^ (ptr[3] & 0x18);
	
	uint8_t l3 = l0 ^ (ptr[4] & 0xE7);
    uint8_t h3 = h0 ^ (ptr[4] & 0x18);
...
void VideoDecoder::Decode4x4PrimitiveBlend(const uint8_t* ptr)
{
	uint16_t c0 = (const uint16_t*)(ptr);
	uint16_t c1 = (const uint16_t*)(ptr + 2);

	uint8_t r0 = (ptr[0] >> 3);
    uint8_t g0 = (ptr[1] >> 5) | ((ptr[0] & 3)<<5);
    uint8_t b0 = (ptr[1] & 0x1F);
	
	uint8_t r3 = (ptr[2] >> 3);
    uint8_t g3 = (ptr[3] >> 5) | ((ptr[2] & 3)<<5);
    uint8_t b3 = (ptr[3] & 0x1F);
	...
	uint8_t l1 = (r1 << 3) | (g1 >> 3);
    uint8_t h1 = b1 | (g1 << 5);
	
	uint8_t l2 = (r2 << 3) | (g2 >> 3);
    uint8_t h2 = b2 | (g2 << 5);
...
void VideoDecoder::Decode8x8PrimitiveBlend( const uint8_t* ptr )
{
	uint8_t id = ptr[0];
